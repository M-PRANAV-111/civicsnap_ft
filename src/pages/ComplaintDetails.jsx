...
export default function ComplaintDetails() {
  ...
  useEffect(() => {
    ...
    fetchComplaintById(id)
      ...
      .catch((error) => {
        if (mounted) {
          setLoadError(error.message || 'Unable to load complaint details.');
        }
      })
      .finally(() => {
        if (mounted) {
  ...
  if (loading && !complaint) {
    return (
      <div className="screen bg-cs-bg items-center justify-center gap-4">
        ...
        <p className="text-cs-muted text-sm">Loading complaint details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="screen bg-cs-bg items-center justify-center gap-4 px-6">
        ...
        <div className="text-center">
          <p className="text-cs-ink text-sm font-medium">Could not load complaint</p>
          ...
          {complaint.status === 'Resolved' && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-emerald-700 text-sm font-semibold">Resolution</p>
              </div>
              {complaint.resolutionProofUrl ? (
                ...
                <p className="text-emerald-700/80 text-sm">Marked as resolved by the assigned officer.</p>
              )}
            </div>
          )}
